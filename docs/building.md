...
The same applies to anything else measuring rendered pixels. `uplink-docs --check`
compares render-SHAPE hashes and has the same split, which is why pages are
regenerated by `uplink-docs.yml` on Linux rather than locally.
