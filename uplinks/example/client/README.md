...
*42 publishes since load: the needle two-thirds through its sweep, the true total in the centre*

![42 publishes since load: the needle two-thirds through its sweep, the true total in the centre](docs/assets/mid-sweep--min.png)

*42 publishes since load: the needle two-thirds through its sweep, the true total in the centre*

## Sections added to other widgets

### `example-cadence-section` into `space-center-status.sections`

- Reads: `example.heartbeat`
- Only while present: `example`
...
*The example Uplink section as it renders in a host panel: a badge, the last publish, and the count*

## Data contributed to other widgets

### `example:heartbeat-blob` into `system-view.entities`

- Computed from: `system.bodies`, `example.heartbeat`
- Only while present: `example`
