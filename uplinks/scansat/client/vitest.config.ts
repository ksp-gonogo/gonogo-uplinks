...
export default defineConfig({
  test: {
    ...
    // vitest's default list, and the sdk tarball ships its own `src` with 140
    // test files in it: the unanchored glob let every one of them be collected
    // and the run died before reaching this Uplink's own tests.
    ...
    // pre-bundles a linked dependency, so this is what the first-party clients
    // get for free. Installed normally, as any author outside that repo installs
